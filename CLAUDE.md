...
- `internal` fields and properties are **not serialized** by Newtonsoft.Json by default (`JToken.FromObject` and `JsonConvert` only visit `public` members). Any `internal` field that must survive serialization (e.g. on `GameData`, `GameDataEntry`) must carry `[JsonProperty]`.
